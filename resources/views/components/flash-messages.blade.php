...
@if (session('success') || session('error') || session('warning') || session('info'))
    <div id="flash-container" class="fixed top-4 right-4 z-[9999] space-y-2" style="z-index: 9999;">
        @if (session('success'))
        ...
        @if (session('warning'))
            <div class="flash-message bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-lg shadow-lg max-w-sm" role="alert">
                <div class="flex items-center">
                    ...
                    <button type="button" class="ml-3 -mx-1.5 -my-1.5 text-yellow-500 hover:text-yellow-600 rounded-lg focus:ring-2 focus:ring-yellow-300 p-1.5 hover:bg-yellow-200 inline-flex items-center justify-center h-8 w-8" onclick="this.parentElement.parentElement.remove()">
                        <x-fas-times class="w-3 h-3" />
                    </button>
                </div>
            </div>
        ...
        @if (session('info'))
            <div class="flash-message bg-blue-100 border border-blue-400 text-blue-700 px-4 py-3 rounded-lg shadow-lg max-w-sm" role="alert">
                <div class="flex items-center">
                    ...
                    <button type="button" class="ml-3 -mx-1.5 -my-1.5 text-blue-500 hover:text-blue-600 rounded-lg focus:ring-2 focus:ring-blue-300 p-1.5 hover:bg-blue-200 inline-flex items-center justify-center h-8 w-8" onclick="this.parentElement.parentElement.remove()">
                        <x-fas-times class="w-3 h-3" />
                    </button>
                </div>
            </div>
